...
public class SwerveModule extends SubsystemBase {
  ...
  // boolean driveMotorIsReversed, boolean turnMotorIsReversed, int absEncoderID,
  // double absEncoderOffsetRad, boolean isTurnedReverse
  ) {
    ...
    turnPIDController = new PIDController(Constants.Drivetrain.kPTurnMotor, Constants.Drivetrain.kITurnMotor,
        Constants.Drivetrain.kDTurnMotor);
  ...
  public double getTurnPosition() {
    double posRad = turnMotor.getSelectedSensorPosition() / Constants.Drivetrain.kTurningMotorGearRatio % 2048 / 2048
        * Math.PI * 2;
    if (posRad < 0) {
      posRad += Math.PI * 2;
    }
  ...
  // // turnMotor.setSelectedSensorPosition((getAbsEncoderRad() / (2 * Math.PI)) *
  // 2048 * Constants.Drivetrain.kTurningMotorGearRatio);
  // double absAngle = absEncoder.getAbsolutePosition() % 360.0;
  // double absPercent = absAngle / 360.0;
  // double absPercentWithRatio = absPercent *
  // Constants.Drivetrain.kTurningMotorGearRatio;
