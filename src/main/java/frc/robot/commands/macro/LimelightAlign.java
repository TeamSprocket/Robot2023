...
public class LimelightAlign extends CommandBase {
  ...
  public LimelightAlign(SwerveDrive swerveDrive, double duration) {
  ...
  public void execute() {
    ...
    double output = pidController.calculate(tXOffset);
    setSpeeds(output);
  ...
  public void setSpeeds(double output) {
    ...
    if (Constants.Drivetrain.IS_FIELD_ORIENTED) {
      ...
      chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
          0, 0, output, new Rotation2d(headingRad));
    } else { 
  ...
  public boolean isFinished() {
    return timer.get() >= duration || tValid == 0.0;
  }
