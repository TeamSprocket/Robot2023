...
public class TrajectoryAutonomousStep extends AbstractAutonomousStep {
    ...
    public void execute(TemplateAuto templateAuto,
        ...
        swerveDrive.setAutoPath(trajectory); //Send the auto to our drive class to be executed
        // swerveDrive.setAutoRotation(rotations.get(0).rotation);
        // int rotationIndex = 1; // Start at the second rotation (the first is the starting rotation)
        // while (!swerveDrive.isFinished()) { // Wait till the auto is done
        //     if (rotationIndex < rotations.size() &&
        //             swerveDrive.getAutoTime() > rotations.get(rotationIndex).time) {
        //         // We've passed the time for the next rotation
        //         swerveDrive.setAutoRotation(rotations.get(rotationIndex).rotation); //Set the rotation
        //         rotationIndex++; // Increment the rotation index
        //     }

        //     if (!scriptsToExecuteByTime.isEmpty() &&
        //             scriptsToExecuteByTime.get(0).getDelay() <= swerveDrive.getAutoTime()) {
        ...
        //             (swerveDrive.getAutoTime() / trajectory.getTotalTimeSeconds())) {
        //         // We have a script to execute, and it is time to execute it
        ...
        // if (rotationIndex < rotations.size()) {
        //     // swerveDrive.setAutoRotation(rotations.get(rotations.size() - 1).rotation);
        //     swerveDrive.setDriveState(swerveDrive.DriveState.AUTO);
        // }
